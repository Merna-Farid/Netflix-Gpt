...
const SignUp = () => {
  ...
  // returns true/false so callers can react to the result
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (!loggedIn) {
        // fall back so the user isn't stranded
        router.push("/login");
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-cover bg-no-repeat flex items-center justify-center bg-black loginContainer">
      <div className="max-w-[480px] w-full bg-[#000000b3] rounded-sm py-12 px-16 font-bold text-[2rem] text-white flex flex-col gap-5 z-50">
        ...
        <div className="flex flex-col gap-1">
          ...
          />
          {errors.name && <span className="text-red-500 text-sm font-normal">{errors.name}</span>}
        </div>
        ...
        <div>
          <span className="text-[#ffffffb3] text-[1rem]">Already have an account? </span>
          <Link href="/login" className="text-white text-[1rem]">Log In</Link>
        </div>
      </div>
    </div>
  );
